  ...
  },
  {
    ...
    svg: (
      ...
      >
        ...
        />
        <path
          d="M20.6102 8.96898C21.4612 9.70251 22 12C22 12.7031 21.8186 13.3638 21.5 13.9379"
          stroke="#14427c"
          strokeWidth="2.2"
          strokeLinecap="round"
          strokeLinejoin="round"
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-10">
          ...
          {items.map((item) => (
            ...
            >
              <div className="flex flex-col items-center justify-start gap-3 text-center rounded-2xl p-6 h-full">
                <div className="flex items-center justify-center w-24 md:w-[150px] h-24 md:h-[150px]">
                  {item.svg}
                </div>
                <p className="font-bold text-black text-sm md:text-xl leading-snug">
                  {item.title}
                </p>
              </div>
            </div>
      ...
      <div className="flex items-center justify-center gap-6 mt-8">
        <button
          ...
          disabled={index === 0}
          aria-label="Anterior"
          className="w-10 h-10 bg-gray-400 rounded-full border border-white/20 flex items-center justify-center text-black transition-opacity duration-200 disabled:opacity-30 disabled:cursor-not-allowed hover:border-white/50"
        >
          ...
          {Array.from({ length: maxIndex + 1 }).map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-700  ${
                i === index ? "w-7 bg-black" : "w-2 bg-gray-400"
              }`}
            />
          ))}
        </div>

...
const Tagline = ({
  ...
  <div
    className={`flex flex-col sm:flex-row justify-between items-start sm:items-center w-full mt-2 md:mt-10 gap-1 md:gap-2 ${light ? "text-white" : "text-[#393939]"}`}
  >
    <p className="text-sm md:text-2xl font-bold">La planta de asfalto más</p>
...
const CARDS: CardData[] = [
  {
    ...
    content: (
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 pt-10 md:pt-20">
        ...
        <p className="text-sm md:text-2xl">
          desde los más <span className="text-xs md:text-sm">pequeños</span>
        </p>
        <p className="text-sm md:text-2xl">
          hasta los{" "}
          <span className="font-bold text-base md:text-3xl">grandes</span>
        </p>
        ...
        <div className="space-y-2 lg:space-y-4 text-sm sm:text-base lg:text-lg leading-relaxed mt-4">
          <h2 className="font-bold text-sm md:text-2xl">
            Sabemos que la operación de muchos equipos depende de la
            confiabilidad de su planta de asfalto
          </h2>
          <p className="text-sm md:text-2xl">
            Por eso nos enfocamos en ofrecer esta planta de asfalto, que sobre
            todo está diseñada y fabricada para una producción continua sin
            costosas fallas imprevistas y un diseño superior para realzar el
            nombre de nuestros clientes.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-3 place-items-center gap-3 sm:gap-5 lg:gap-8 w-full">
          {items.map((item, index) => (
            ...
            >
              ...
              <p className="text-[10px] sm:text-xs lg:text-base font-bold leading-tight mt-1">
                {item.text}
              </p>
            </div>
          ))}
  ...
  },
  {
    ...
    content: (
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
        <h2 className="text-2xl md:text-5xl font-bold tracking-tight mb-1 lg:mb-6 text-white">
          100% fabricantes
        </h2>
        <p className="text-white font-bold text-sm md:text-xl">
          Con nosotros usted trabaja directamente con el
        </p>
        <p className="text-white font-bold text-sm md:text-xl">
          fabricante -sin intermediarios ni distribuidores.
        </p>
...
export default function DominoCards() {
  ...
  useEffect(() => {
    ...
    const totalScrollPx =
      CARDS.reduce((sum, c) => sum + c.scrollWeight, 0) * vh;
    ...
    cards.forEach((card, i) => {
      ...
      tl.to(
        card,
        { y: 0, ease: "power2.out", duration: CARDS[i].scrollWeight },
        accumulated,
      );
      accumulated += CARDS[i].scrollWeight;
    });
    ...
    <section className="relative w-full">
      ...
      >
        ...
        >
          <img
            src={render.src}
            alt="Planta de asfalto"
            className="w-full h-auto max-h-[28vh] sm:max-h-[35vh] lg:max-h-none object-contain"
          />
        </div>
